...
const Vendor = () => {
    ...
    useEffect(() => {
        ...
        axios.get('http://emkl-abb.virtusrox.me/api/select/vendor.php').then(response => {
            ...
            $(document).ready(function () {
                ...
                if (lengthData > 0) {
                    ...
                    $('#input-kode').val("V" + formatKode.substring(0, formatKode.length - lastKode.length) + (parseInt(lastKode) + 1));
                } else {
                    $('#input-kode').val("V" + formatKode);
                }
            });
    ...
    const hapusData = (kode) => {
        ...
        axios.post('http://emkl-abb.virtusrox.me/api/delete/vendor.php', data, config).then(response => {
    ...
    const simpanData = () => {
        if (checkInputValidity('form')) {
            ...
            axios.post('http://emkl-abb.virtusrox.me/api/insert/vendor.php', data, config).then(response => {
                window.location.reload();
            });
        }
    }
    ...
    const terapkanData = (kode) => {
        if (checkInputValidity('form')) {
            ...
            axios.post('http://emkl-abb.virtusrox.me/api/update/vendor.php', data, config).then(response => {
    ...
    return (
        <div className="active content overflow-auto">
            <p className="fw-bold text-secondary text-size-10">Vendor</p>
            <p className="text-secondary">Master / <span className="fw-bold primary-text-color">Vendor</span></p>
            <form id="form" className="card-form">
                <div className="p-4">
                    <div className="align-items-center d-flex flex-wrap pb-3 pb-md-2 px-0">
                        <label htmlFor="input-kode" className="col-12 col-lg-2 col-md-3 pb-2 pb-md-0">Kode Vendor</label>
                        <div className="col col-lg-2 col-md-3">
                            <input type="text" id="input-kode" name="input-kode" className="form-control" placeholder="Kode Vendor" maxLength={10} readOnly required />
                        </div>
                    </div>
                    <div className="align-items-center d-flex flex-wrap pb-3 pb-md-2 px-0">
                        <label htmlFor="input-nama" className="col-12 col-lg-2 col-md-3 pb-2 pb-md-0">Nama Vendor</label>
                        <div className="col">
                            <input type="text" id="input-nama" name="input-nama" className="form-control" placeholder="Nama Vendor" maxLength={100} required />
                        </div>
                    </div>
                    <div className="align-items-center d-flex flex-wrap pb-3 pb-md-2 px-0">
                        <label htmlFor="input-jenis" className="col-12 col-lg-2 col-md-3 pb-2 pb-md-0">Jenis Vendor</label>
                        <div className="col">
                            <input type="text" id="input-jenis" name="input-jenis" className="form-control" placeholder="Jenis Vendor" maxLength={100} required />
                        </div>
                    </div>
                    <div className="align-items-center d-flex flex-wrap pb-3 pb-md-2 px-0">
                            ...
                            {vendor.map((data, index) =>
                                <tr key={data.id}>
                                    ...
                                    <td className="text-center">
                                        ...
                                        <div><input type="text" id={"edit-telepon-" + data.kode} name={"edit-telepon-" + data.kode} className="d-none form-control" defaultValue={data.telepon} /></div>
                                    </td>
                                    <td>
